...
_SYSTEM_PROMPT = (
    ...
    "домыслов. Отвечай только готовым описанием на русском языке без служебных "
    "фраз, заголовков или списков, за исключением завершающей строки «Ключевые "
    "направления: …»."
)
...
_INSTRUCTION_BLOCK = dedent(
    ...
    Для каждой темы приведи одно-два развернутых предложения с перечислением конкретных фактов, продуктов, услуг, технологий, процессов и типов клиентов.
    ...
    В текст включай отраслевые термины и синонимы, которые помогут классифицировать направление деятельности компании.
    Указывай только факты, даты и цифры из исходного текста. Не делай выводов, оценок или предположений.
    В конце добавь отдельную строку «Ключевые направления: …» с перечислением 6–10 коротких ключевых фраз через запятую, используя формулировки из текста и их релевантные синонимы.
    """
).strip()
