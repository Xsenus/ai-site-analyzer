...
_SYSTEM_PROMPT = (
    ...
    "входного текста. Отвечай только готовым описанием на русском языке без "
    "дополнительных пометок, заголовков или служебных фраз."
)

_INSTRUCTION_BLOCK = dedent(
    """
    Составь самый подробный обзор компании на русском языке, строго по фактам из исходного текста.
    Дай цельный рассказ без заголовков и нумерованных списков.
    Отрази по порядку: профиль и специализацию, ключевые продукты и услуги, используемые технологии и процессы,
    отрасли или клиентов, сертификаты и стандарты, условия сервиса и сотрудничества, уникальные особенности.
    Если каких-то сведений нет, прямо укажи «Информация о … не указана».
    Не добавляй ни одного факта, которого нет во входном тексте. Без маркетинговых клише и домыслов.
    """
).strip()
