...
_NON_PRODUCTION_EQUIPMENT_PATTERNS = (
    "цифровая платформа",
...
_STEEL_EQUIPMENT_CANONICAL = (
    "Нагревательная печь для слябов (шагающие балки)",
    "Рольганг выдачи и транспортные ролики",
    "Чистовая клеть стана",
    "Черновая клеть стана",
    "Моталка горячего рулона (оправка и прижим)",
    "Гидросбив окалины (дескейлер) — внешние узлы",
    "Намоточно-размоточные агрегаты",
    "Система натяжения и бридлы",
    "Камера нанесения покрытия (наносные валы)",
...
PRODCLASS_SCORE= уровень смысловой связи (сходства) текста сайта компании с выбранным PRODCLASS; число с точкой в диапазоне 0.00–1.00 с 3-4 знаками после запятой (например, 0.7831);
...
Не включай в EQUIPMENT_SITE цифровые платформы, сайты, личные кабинеты, лаборатории, центры компетенций, отделы, услуги, проекты и организационные сущности — это не физическое производственное оборудование.
...
GOODS_TYPE должен быть конкретным товарным направлением ТН ВЭД с материалом/отраслью, а не общим словом. Например, для металлургии не пиши "Сталь", "Прокат" или "Заготовки"; пиши "Прокат плоский горячекатаный шириной 600 мм и более из нелегированной стали", "Прокат плоский холоднокатаный шириной 600 мм и более из нелегированной стали", "Полуфабрикаты из железа или нелегированной стали (включая слябы)".
...
def _is_non_production_equipment_item(item: str) -> bool:
    ...
    if not normalized:
        return True
    return any(pattern in normalized for pattern in _NON_PRODUCTION_EQUIPMENT_PATTERNS)


def _filter_equipment_items(items: List[str]) -> List[str]:
    return [item for item in _dedup_ordered(items) if not _is_non_production_equipment_item(item)]


def _is_steel_context(*values: str) -> bool:
    context = " ".join(value for value in values if value).casefold()
...
def refine_goods_type_items(
    ...
    *,
    ...
    refined = _dedup_ordered(items)
    if not refined:
        return refined

    if not _is_steel_context(okved, description, site_text[:4000], " ".join(refined)):
        return refined

    has_steel_goods = any(
        any(keyword in item.casefold() for keyword in _STEEL_GENERIC_GOODS_KEYWORDS)
        for item in refined
    )
    if not has_steel_goods:
        return refined

    # Для металлургии общие слова вроде "сталь", "прокат" и "заготовки" дают ложные совпадения
    # с пищевыми/строительными товарами. Раскрываем их до конкретных направлений ТН ВЭД.
    return _dedup_ordered(list(_STEEL_GOODS_TYPE_CANONICAL) + refined)


def build_catalog_query_context(
    *,
    company_name: str = "",
    okved: str = "",
    description: str = "",
    ...
    parts = [
        f"Компания: {company_name.strip()}" if company_name and company_name.strip() else "",
        f"ОКВЭД: {okved.strip()}" if okved and okved.strip() else "",
        f"Описание: {description.strip()}" if description and description.strip() else "",
    ]
    return " | ".join(part for part in parts if part)[:1200]


def _catalog_query_text(item: str, query_context: str | None) -> str:
    item = item.strip()
    context = (query_context or "").strip()
    if not context:
        return item
...
async def parse_openai_answer(answer: str, text_par_for_fallback: str, embed_model: str) -> Dict[str, Any]:
    ...
    data["EQUIPMENT_LIST"] = _filter_equipment_items(_split(data["EQUIPMENT_RAW"]))
