...
Поле `prompt` содержит инструкцию OpenAI ("Запрос №2") вернуть только одно число — идентификатор класса производства из справочника `IB_PRODCLASS`, который ближе всего к переданному ОКВЭД. Поле `parsed` содержит итоговый ID класса, а `timings`, `events` и `answer` позволяют проследить работу конвейера и диагностировать ошибки.
