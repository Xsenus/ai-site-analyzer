...
3. **Каталоги** передаются в поле `items`. Для каждого элемента можно указать
   либо `values`, либо `literal`, либо оба варианта. Если у вас уже подготовлена
   строка pgvector, достаточно заполнить только `literal`.
